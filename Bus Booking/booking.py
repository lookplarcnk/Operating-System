...
class user:
    ...
    #Login Function
    def login(self):
        if check_user(self.username.get(), self.password.get()):
    ...
    def new_user(self):
        if check_user(self.n_username.get(), self.n_password.get()):
            ms.showerror('Error!','Username Already Taken!')
        else:
            ms.showinfo('Success!','Account Created!')
            self.log()
            save_user(self.n_username.get(), self.n_password.get())
    ...
    def widgets(self):
        ...
        Label(self.logf, text='Username: ', font=('', 20), pady=5, padx=5, fg='black').grid(sticky=W)
        Entry(self.logf, textvariable=self.username, bd=5, font=('', 15)).grid(row=0, column=1)
        Label(self.logf, text='Password: ', font=('', 20), pady=5, padx=5, fg='black').grid(sticky=W)
        ...
        Label(self.crf, text='Username: ', font=('', 20), pady=5, padx=5, fg='black').grid(sticky=W)
        Entry(self.crf, textvariable=self.n_username, bd=5, font=('', 15)).grid(row=0, column=1)
        Label(self.crf, text='Password: ', font=('', 20), pady=5, padx=5, fg='black').grid(sticky=W)
...
class travel:
    ...
    #==========================================Define Functiom==================================================
        ...
        def Receiptt():
            if reset_counter is 0 and Firstname.get()!="" and Surname.get()!="" and Address.get()!="" and Postcode.get()!="" and Mobile.get()!="" and Telephone.get()!="" and Email.get()!="":
                ...
                self.txtReceipt1.insert(END,'Standard Bus:\n')
                self.txtReceipt2.insert(END, Standard.get() + "\n")
                self.txtReceipt1.insert(END,'Trailers Bus:\n')
                self.txtReceipt2.insert(END, PrimeSedan.get() + "\n")
                self.txtReceipt1.insert(END,'Air Con Bus:\n')
            ...
            save_booking_details("Receipt Ref: " + Receipt_Ref.get() + "\n" +
                         "Date: " + DateofOrder.get() + "\n" +
                         "Taxi No: TR " + Receipt_Ref.get() + " BW\n" +
                         "Firstname: " + Firstname.get() + "\n" +
                         "Surname: " + Surname.get() + "\n" +
                         "Address: " + Address.get() + "\n" +
                         "Postal Code: " + Postcode.get() + "\n" +
                         ...
                         "To: " + varl2.get() + "\n" +
                         "Pooling: " + varl3.get() + "\n" +
                         "Standard Bus: " + Standard.get() + "\n" +
                         "Trailers Bus: " + PrimeSedan.get() + "\n" +
                         "Air Con Bus: " + PremiumSedan.get() + "\n" +
                         "Paid: " + PaidTax.get() + "\n" +
        ...
        def Kilo():
            if var2.get() == 0:
                ...
                if varl1.get() == "Bangkok":
                    switch ={"Chiang Mai": 10,"Phuket": 8,"Chonburi":6,"Bangkok": 0}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Chiang Mai":
                    switch ={"Chiang Mai": 0,"Phuket": 2,"Chonburi":5,"Bangkok": 10}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Phuket":
                    switch ={"Chiang Mai": 2,"Phuket": 0,"Chonburi":3,"Bangkok": 8}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Chonburi":
                    switch ={"Chiang Mai": 5,"Phuket": 3,"Chonburi":0,"Bangkok": 6}
                    Km.set(switch[varl2.get()])        
        ...
        def selectCar():
            ...
            if carType.get() == 1:
                ...
                carPrice = float(8)
                Standard.set("Rs "+ str(carPrice) + " - Standard Bus")
            elif carType.get() == 2:
                self.txtStandard.configure(state=DISABLED)
                Standard.set("0")
                self.txtPremiumSedan.configure(state=DISABLED)
                ...
                carPrice = float(10)
                PrimeSedan.set("Rs "+ str(carPrice) + " - Trailers Bus")
            else:
                self.txtStandard.configure(state=DISABLED)
                Standard.set("0")
                self.txtPrimeSedan.configure(state=DISABLED)
                ...
                carPrice = float(15)
                PremiumSedan.set("Rs "+ str(carPrice) + " - Air Con Bus")
   ...
   #================================================mainframe========================================================================
        ...
        self.lblTitle=Label(Tops,font=('arial',50,'bold'),text=" Bus Booking System ")
        self.lblTitle.grid()
    ...
    #===============================================Taxi Information==============================================================
        ...
        self.cboPickup['value']=('','Phuket','Chonburi','Chiang Mai','Bangkok')
        self.cboPickup.current(0)
        ...
        self.cboDrop['value']=('','Bangkok','Chiang Mai','Phuket','Chonburi')
        self.cboDrop.current(0)
    ...
    #===============================================Bus Information==============================================================
        ...
        self.chkTaxiTax=Checkbutton(TravelFrame,text="Tax(Base Charge) *",variable = var1, onvalue=1, offvalue=0,font=('arial',16,'bold'),command=Taxi_Tax).grid(row=3, column=0, sticky=W)
    ...
    #==========================================================bus select=======================================================================
        ...
        self.chkStandard = Radiobutton(Book_Frame, text="Standard Bus", value=1, variable=carType, font=('arial', 14, 'bold'), command=selectCar).grid(row=0, column=0, sticky=W)
        self.chkPrimeSedand = Radiobutton(Book_Frame, text="Trailers Bus", value=2, variable=carType, font=('arial', 14, 'bold'), command=selectCar).grid(row=1, column=0, sticky=W)
        self.chkPremiumSedan = Radiobutton(Book_Frame, text="Air Con Bus", value=3, variable=carType, font=('arial', 14, 'bold'), command=selectCar).grid(row=2, column=0)
